...
unsigned int bucket_index(int key) {
    return (unsigned int)key % BUCKET_COUNT;
}
```
...
int table_put(table_t *table, int key, int val) {
    unsigned int index = bucket_index(key);
    for (entry_t *entry = table->buckets[index]; entry != NULL; entry = entry->next) {
        if (entry->key == key) {
            entry->val = val;
            return 1;
        }
...
Write `size_t table_size(const table_t *table)` to return the stored entry count.
